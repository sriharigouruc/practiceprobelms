    ...
    WITH ForecastWeekSeries AS (SELECT (DR.WeekYear::VARCHAR || LPAD(DR.WeekNumber::TEXT, 2, '0'))::INTEGER AS ForecastWeek,
         ...
         BestModelList AS (SELECT ESF.zipcode,
                                  ...
                                  CASE WHEN ESF.forecastmodelid = 6
                                           THEN RNF.ForecastResult
                                       ELSE RF.ForecastResult 
                           ...
                           FROM   (SELECT ESF2.zipcode,
                                  ...
                                  CROSS JOIN LATERAL (SELECT RunNaiveForecast(ESF.forecastrentweek1, _Weeks) AS ForecastResult) RNF
                                  CROSS JOIN LATERAL (SELECT RunForecast(ESF.forecastfactorlist, ESF.forecastmodelid, _Weeks) AS ForecastResult) RF
